...
void readCSVNominal(String fileName) throws Exception {
  ...
  options[0]="-R";
  options[1]="3";//+(attrNum-1); //range of variables to make numeric
  try {
    // have to get the data out of Instances
...
PGraphics getRegModelImage(PGraphics pg, Classifier cls, Instances training) {
  color colors[] = {
    color(155, 89, 182), color(63, 195, 128), color(214, 69, 65), 
    color(82, 179, 217), color(244, 208, 63), color(242, 121, 53), 
    color(0, 121, 53), color(128, 128, 0), color(52, 0, 128), 
  ...
  for (int x = 0; x < pg.width; x++) {
    for (int y = 0; y < pg.height; y++) {
      ...
      int color1Index = max( floor(classification), 0);
      int color2Index = color1Index+1;
      float diff = max( classification-color1Index, 0.);
